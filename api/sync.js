...
export default async function handler(req, res) {
  // Configurar CORS headers para permitir requisições de outros domínios se necessário
  ...
  if (!supabaseServiceKey) {
    return res.status(500).json({ error: "Configuração do Supabase incompleta (SUPABASE_SERVICE_ROLE_KEY ausente)." });
  }
  ...
  try {
    ...
    console.error("Erro na Vercel Function:", err.message);
    return res.status(500).json({ error: "Falha na sincronização automática: " + err.message });
  }
}
